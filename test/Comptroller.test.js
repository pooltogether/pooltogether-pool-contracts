...
describe('Comptroller', () => {
  ...
  describe('deactivateBalanceDrip()', () => {
    ...
    it('should remove a balance drip', async () => {
      await expect(comptroller.deactivateBalanceDrip(prizePoolAddress, measure.address, dripToken.address, SENTINAL))
    ...
    it('should allow only the owner to remove drips', async () => {
      await expect(comptroller2.deactivateBalanceDrip(prizePoolAddress, measure.address, dripToken.address, SENTINAL)).to.be.revertedWith("Ownable: caller is not the owner")
    })
  })
