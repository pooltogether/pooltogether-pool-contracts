...
describe('Ticket Interest Feature', () => {
  ...
  it('should earn interest on held tickets', async () => {
    ...
    await env.expectUserToHaveTicketCredit({ user: 1, credit: 100 })
  })
...
*/
    ...
    */
    ...
    await env.expectUserToHaveTicketCredit({ user: 1, credit: 80 })

    // given that the user began with a rate of 1.5, we expect (end rate - start rate * tokens = 20) => (endRate - 1.5 * 100 = 20) => endRate = 1.7
    await env.expectUserToHaveTicketCredit({ user: 2, credit: 20 }) // user 1 should have 150/250 of 50, and user 2 has 100/250 of 50
  })
