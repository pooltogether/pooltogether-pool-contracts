...
abstract contract PrizePool is OwnableUpgradeSafe, RelayRecipient, ReentrancyGuardUpgradeSafe, TokenControllerInterface {
  ...
  /// @notice Adds credit for a user for a controlled token.  The credit is capped at the credit limit for that controlled token.
  /// @param user The user to whom the credit should be allocated
  /// @param controlledToken The controlled token to credit for
  /// @param controlledTokenBalance The user's balance of the controlled token
  /// @param credit The amount of credit to give to the user
  function _addCredit(address user, address controlledToken, uint256 controlledTokenBalance, uint256 credit) internal {
    CreditBalance storage creditBalance = tokenCreditBalances[controlledToken][user];
    uint128 newBalance = _applyCreditLimit(controlledToken, controlledTokenBalance, uint256(creditBalance.balance).add(credit)).toUint128();
  ...
  function _accrueCredit(address user, address controlledToken, uint256 controlledTokenBalance) internal {
    ...
    _addCredit(user, controlledToken, controlledTokenBalance, credit);
  }
    ...
    returns (uint256)
  {
    ...
    */
    ...
    uint256 totalEarlyExitFee = _calculateEarlyExitFee(controlledToken, controlledTokenBalance);
    uint256 remainingEarlyExitFee;

    // if the credit balance is less than the total fees for the account
    if (tokenCreditBalances[controlledToken][from].balance < totalEarlyExitFee) {
      remainingEarlyExitFee = totalEarlyExitFee.sub(tokenCreditBalances[controlledToken][from].balance);
    }

    uint256 earlyExitFee = _calculateEarlyExitFee(controlledToken, amount);
    ...
    if (earlyExitFee > remainingEarlyExitFee) {
      ...
      creditBurned = earlyExitFee.sub(remainingEarlyExitFee);
      // the remainder is the actual fee
      earlyExitFee = earlyExitFee.sub(creditBurned);
    }
